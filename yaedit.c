...
void showstatus(void)
{
    ...
    color_c1(); printw( " F5" );  color_c2(); printw( "RUN! " );

    color_c1(); printw( " F7" );  color_c2(); printw( "Mode " );
  ...
  if ( strcmp( file_name, "" ) == 0 )   strncpy( file_name, "noname.c" , STRMAX );
	...
	if (argv >= 2) {
		...
		case KEY_F(5):
			...
			k_save_quick();
			color_c2();
                        if ( fexist( file_name ) == 1 )  make_ya( file_name );
			break;
